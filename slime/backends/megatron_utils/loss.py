...
def decoupled_policy_loss_function(
    ...
    # 🔧 FIX: For off-policy GRPO, behavior_log_probs should be the log_probs from the policy
    # that generated the samples (i.e., rollout_log_probs), not the log_probs computed during training.
    # When MAX_STALENESS=0, this ensures importance weight = 1 (on-policy equivalence).
    max_staleness = getattr(args, 'max_staleness', -1)
    ...
    if "behavior_log_probs" in batch and batch["behavior_log_probs"] is not None:
        ...
        # 🔧 FIX: Prioritize rollout_log_probs over log_probs for behavior policy
        # rollout_log_probs are the log_probs from the policy that generated the samples
        # log_probs are computed during training (may differ from rollout policy)
        if "rollout_log_probs" in batch and batch["rollout_log_probs"] is not None:
            # Use rollout_log_probs (generated by behavior policy) as behavior policy
            behavior_log_probs = batch["rollout_log_probs"]
            if max_staleness == 0 and is_megatron_main_rank():
                print(f"[Decoupled Policy Loss] MAX_STALENESS=0: Using rollout_log_probs as behavior_log_probs "
                      f"for on-policy equivalence.")
        elif "log_probs" in batch and batch["log_probs"] is not None:
            # Fallback: use old_actor log_probs (computed in train_actor) as behavior policy
            # This is less ideal but acceptable if rollout_log_probs are not available
            behavior_log_probs = batch["log_probs"]
            if max_staleness == 0 and is_megatron_main_rank():
                print(f"[Decoupled Policy Loss] WARNING: MAX_STALENESS=0 but using log_probs instead of "
                      f"rollout_log_probs. This may cause importance weight != 1.")
        else:
            raise ValueError(
    ...
    # 🔧 FIX: Validate importance weights when MAX_STALENESS=0
    # When MAX_STALENESS=0, we should only sample data from current policy version,
    # which means proximal_log_probs should equal behavior_log_probs (importance weight = 1)
    if max_staleness == 0:
        # Compute mean importance weight for monitoring
        all_masks = torch.cat(batch["loss_masks"], dim=0)
        masked_weights = importance_weights * all_masks
        num_valid = all_masks.sum()
        if num_valid > 0:
            mean_importance_weight = masked_weights.sum() / num_valid
            # Check if importance weights are close to 1 (within tolerance)
            tolerance = 0.01
            if is_megatron_main_rank():
                if torch.abs(mean_importance_weight - 1.0) > tolerance:
                    print(f"[Decoupled Policy Loss] WARNING: MAX_STALENESS=0 but mean importance weight = "
                          f"{mean_importance_weight.item():.4f} (expected ~1.0). "
                          f"This may indicate a bug in behavior_log_probs or proximal_log_probs computation.")
                else:
                    print(f"[Decoupled Policy Loss] MAX_STALENESS=0: Mean importance weight = "
                          f"{mean_importance_weight.item():.4f} (on-policy equivalence verified).")
