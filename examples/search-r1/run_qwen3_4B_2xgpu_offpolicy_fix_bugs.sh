...
# Random sampling from buffer (uniform distribution)
# For priority-based sampling, change BUFFER_SAMPLING_STRATEGY to "priority" and uncomment priority-related args

BUFFER_SAMPLING_ARGS=(
   --buffer-sampling-strategy ${BUFFER_SAMPLING_STRATEGY}
   # Priority-related args (only used when strategy is "priority")
   # --buffer-priority-metric reward
   # --buffer-priority-weight 1.0  # Weight for reward in priority scoring
   --buffer-staleness-penalty 0  # Penalty for stale samples
   # Note: For priority strategy, normalize-priority-scores and priority-norm-method use default values (True and 'minmax')
   # They are not configurable via command line arguments yet
   # --buffer-random-seed 42  # Uncomment for reproducible results
)
...
export WANDB_KEY="5e7332a98e2c4c7d2252cff2d58bd1d3948899e3"  # Replace with your key
